...
install:
	...
	@if [ ! -f $(TARGET_OUT)/install_SGX$(SGXCORE)_$(SGX_CORE_REV).sh ]; then \
		echo; \
		echo "** install_SGX$(SGXCORE)_$(SGX_CORE_REV).sh not found in $(TARGET_OUT)."; \
		echo "** Cannot continue with install."; \
		echo; \
		exit 1; \
	fi
	@cd $(TARGET_OUT) && ./install_SGX$(SGXCORE)_$(SGX_CORE_REV).sh
endif
