...
#
# $Revision: 1.6 $
# $Log: xorg_cache_tarball_zlib.mk $
#

$(call target-build-only,xorg_cache_tarball_zlib)
...
#$(info [$(THIS_MODULE)] $(Host_or_target) xorg_cache_tarball_zlib: $(MODULE_TARGETS))

define zlib-configure
$(if $(V),,@)pwd=$$PWD && \
	cd $(MODULE_INTERMEDIATES_DIR) && \
		CC="$(patsubst @%,%,$(CC))" \
		AR="$(patsubst @%,%,$(AR))" \
		CFLAGS="$(ALL_XORG_CFLAGS)" \
			./configure --prefix=$(XORG_PREFIX) --shared && \
	cd $$pwd
endef

define zlib-install
$(zlib-make) DESTDIR="$(MODULE_DESTDIR)" install
