...
//SUPPORT_FORCE_FLIP_WORKAROUND
		if(ui32TriesLeft == (ui32MaxRetries / 2))
		{
			PVRSRV_MISC_INFO sMiscInfo;

			PVR_DPF((PVR_DBG_ERROR, "WaitUntilResourceIsNotNeeded: Trying force-flip workaround"));

			sMiscInfo.ui32StateRequest = PVRSRV_MISC_INFO_FORCE_SWAP_TO_SYSTEM_PRESENT;
			PVRSRVGetMiscInfo(psMgr->psDevData->psConnection, &sMiscInfo);
		}
//SUPPORT_FORCE_FLIP_WORKAROUND
		if(!ui32TriesLeft)
		{
			PVRSRVClientEvent(PVRSRV_CLIENT_EVENT_HWTIMEOUT, psMgr->psDevData, IMG_NULL);
			return IMG_FALSE;
		}

		if(PVRSRVEventObjectWait(psMgr->psDevData->psConnection, psMgr->hOSEvent) != PVRSRV_OK)
		{
