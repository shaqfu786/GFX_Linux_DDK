											 ...
											 IMG_BOOL bTerminate)
{
	...
	do
	{
		...
		if(!bEnoughSpace)
		{
			...
			if(psBuffer->hOSEvent && !bFirstTry)
			{
				if(PVRSRVEventObjectWait(psBuffer->psDevData->psConnection,
										psBuffer->hOSEvent) != PVRSRV_OK)
				{
					PVR_DPF((PVR_DBG_MESSAGE, "CBUF_GetBufferSpace: PVRSRVEventObjectWait failed"));
				}
			}
		}
		bFirstTry = IMG_FALSE;
...
/***********************************************************************************
 ...
 Inputs             : ps3DDevData, ui32BufferType, hHeapAllocator, ui32Size
 Outputs            : 
