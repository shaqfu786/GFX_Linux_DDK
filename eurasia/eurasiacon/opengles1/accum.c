...
/* PHAS + NOP + TST + LD + WDF + 3 instructions + EMITVCB */
#define GLES1_ACCUM_VERTEX_FIRSTPHAS_SIZE_IN_BYTES (ALIGNCOUNT((EURASIA_USE_INSTRUCTION_SIZE * 9), EURASIA_PDS_DOUTU_PHASE_START_ALIGN)) 
#else
/* PHAS + 3 instructions + EMITVCB */
#define GLES1_ACCUM_VERTEX_FIRSTPHAS_SIZE_IN_BYTES (ALIGNCOUNT((EURASIA_USE_INSTRUCTION_SIZE * 5), EURASIA_PDS_DOUTU_PHASE_START_ALIGN)) 
#endif

#if defined(SGX_FEATURE_VCB)
#define GLES1_ACCUM_VERTEX_SIZE_IN_BYTES	(GLES1_ACCUM_VERTEX_FIRSTPHAS_SIZE_IN_BYTES + (EURASIA_USE_INSTRUCTION_SIZE * 2)) /* + PHAS + EMITVERTEX */
#else
#define GLES1_ACCUM_VERTEX_SIZE_IN_BYTES	(GLES1_ACCUM_VERTEX_FIRSTPHAS_SIZE_IN_BYTES)
#endif /* defined(SGX_FEATURE_VCB) */

#define GLES1_ACCUM_FRAGMENT_SIZE_IN_BYTES	(EURASIA_USE_INSTRUCTION_SIZE * 2) /* PHAS + Instruction */

#else /* defined(SGX_FEATURE_USE_UNLIMITED_PHASES) */

#define GLES1_ACCUM_VERTEX_SIZE_IN_BYTES	(EURASIA_USE_INSTRUCTION_SIZE * 4)
#define GLES1_ACCUM_FRAGMENT_SIZE_IN_BYTES	EURASIA_USE_INSTRUCTION_SIZE
...
************************************************************************************/
IMG_INTERNAL IMG_BOOL InitAccumUSECodeBlocks(GLES1Context *gc)
{
	...
	/*****************************************************
               Fill in USE Vertex Code Heap 
	******************************************************/

	gc->sPrim.psAccumVertexCodeBlock = UCH_CodeHeapAllocate(gc->psSharedState->psUSEVertexCodeHeap,
															GLES1_ACCUM_VERTEX_SIZE_IN_BYTES);
	
	GLES1_INC_COUNT(GLES1_TIMER_USECODEHEAP_VERT_COUNT, GLES1_ACCUM_VERTEX_SIZE_IN_BYTES >> 2);
	...
	GLES1_ASSERT((IMG_UINT32)(pui32Buffer - pui32BufferBase) <= GLES1_ACCUM_VERTEX_SIZE_IN_BYTES >> 2);


	/***************************************************************
	           Generate USE code for HW BGObject 
                Fill in USE Fragment Code Heap 
	***************************************************************/

	gc->sPrim.psHWBGCodeBlock = UCH_CodeHeapAllocate(gc->psSharedState->psUSEFragmentCodeHeap,
													 GLES1_ACCUM_FRAGMENT_SIZE_IN_BYTES);

	GLES1_INC_COUNT(GLES1_TIMER_USECODEHEAP_FRAG_COUNT, GLES1_ACCUM_FRAGMENT_SIZE_IN_BYTES >> 2);
	...
	*/
	...
	/***************************************************************
                 Fill in PDS Fragment Code Heap 
	***************************************************************/

	gc->sPrim.psDummyPixelSecondaryPDSCode = UCH_CodeHeapAllocate(gc->psSharedState->psPDSFragmentCodeHeap, ui32PDSProgramSizeInDWords << 2);

	GLES1_INC_COUNT(GLES1_TIMER_PDSCODEHEAP_FRAG_COUNT, ui32PDSProgramSizeInDWords << 2);
	...
	pui32Buffer = PDSGeneratePixelShaderSAProgram(&sProgram, pui32BufferBase);

	GLES1_ASSERT((IMG_UINT32)(pui32Buffer - pui32BufferBase) == ui32PDSProgramSizeInDWords);
...
************************************************************************************/
IMG_INTERNAL IMG_VOID FreeAccumUSECodeBlocks(GLES1Context *gc)
{
	UCH_CodeHeapFree(gc->sPrim.psAccumVertexCodeBlock);
	UCH_CodeHeapFree(gc->sPrim.psHWBGCodeBlock);
...
************************************************************************************/
static GLES1_MEMERROR SetupVerticesForAccum(GLES1Context *gc,
...
************************************************************************************/
static GLES1_MEMERROR SetupVerticesAndShaderForAccum(GLES1Context *gc, IMG_FLOAT fDepth)
{
...
/***********************************************************************************
 ...
 Returns            : Mem Error
 Description        : Generates PDS code for background object. 
                      SW Object is for accumulation, 
                      HW object is for SPM (Smart Parameter Management).
************************************************************************************/
IMG_INTERNAL GLES1_MEMERROR SetupBGObject(GLES1Context *gc, IMG_BOOL bIsAccumulate, IMG_UINT32 *pui32PDSState)
{
	...
	*/	
	...
	if(IsColorAttachmentTwiddled(gc, gc->sFrameBuffer.psActiveFrameBuffer))
	{
		...
		sTextureImageUnit.ui32TAGControlWord1 |= EURASIA_PDS_DOUTT1_TEXTYPE_ARB_2D                                      |
			                                        (((ui32AccumWidth  - 1) << EURASIA_PDS_DOUTT1_WIDTH_SHIFT)  & ~EURASIA_PDS_DOUTT1_WIDTH_CLRMSK)  |
			                                        (((ui32AccumHeight - 1) << EURASIA_PDS_DOUTT1_HEIGHT_SHIFT) & ~EURASIA_PDS_DOUTT1_HEIGHT_CLRMSK) ;
#endif /* defined(SGX_FEATURE_TAG_POT_TWIDDLE) */
	...
	*/
	...
	if(bIsAccumulate)
	{
		sTextureImageUnit.ui32TAGControlWord2 = (psDrawParams->ui32AccumHWAddress >> EURASIA_PDS_DOUTT2_TEXADDR_ALIGNSHIFT) << EURASIA_PDS_DOUTT2_TEXADDR_SHIFT;
	}
	else
	{
		sTextureImageUnit.ui32TAGControlWord2 = (psDrawParams->ui32HWSurfaceAddress >> EURASIA_PDS_DOUTT2_TEXADDR_ALIGNSHIFT) << EURASIA_PDS_DOUTT2_TEXADDR_SHIFT;
	}
	...
	*/
	...
	ui32PDSProgramSize += (ui32PDSDataSize + ((1 << (EURASIA_PDS_DATASIZE_ALIGNSHIFT - 2)) - 1)) & 
                                            ~((1UL << (EURASIA_PDS_DATASIZE_ALIGNSHIFT - 2)) - 1);
	...
	*/
	...
	GLES1_ASSERT(!(uPDSPixelShaderSecondaryBaseAddr.uiAddr & ((1 << EURASIA_PDS_BASEADD_ALIGNSHIFT) - 1)));
...
************************************************************************************/
IMG_INTERNAL GLES1_MEMERROR SendAccumulateObject(GLES1Context *gc, IMG_BOOL bClearDepth, IMG_FLOAT fDepth)
{
	...
	*pui32Buffer++	= 
	...
	/* ========= */
	*pui32Buffer++ = EURASIA_TATEXFLOAT_TC0_16B;	/* PRQA S 3199 */ /* pui32Buffer is used in the following GLES1_ASSERT. */
