...
************************************************************************************/
static IMG_BOOL HardwareMipGen(GLES1Context *gc,
							   GLESTexture *psTex, 
							   ...
							   IMG_UINT32 *pui32Lod)
{
    ...
    GLES1MemSet(&mipgenQueueTransfer, 0, sizeof(SGX_QUEUETRANSFER));
    ...
    while(ui32MaxDim)
    {
		if (ui32Lod < ui32BaseLevel+GLES1_MAX_TEXTURE_MIPMAP_LEVELS)
		{
			ui32Lod++;
			ui32MaxDim >>= 1;
		}	    
    ...
    *pui32Lod = ui32Lod;	
    ...
    eResult = SGXQueueTransfer(gc->psSysContext->hTransferContext, &mipgenQueueTransfer);
    ...
    if(eResult != PVRSRV_OK)
    {
		...
		if (psSrcSyncInfo)
		{
			...
			if (PVRSRVPollForValue(gc->psSysContext->psConnection,
								   ...
								   &psSrcSyncInfo->psSyncData->ui32WriteOpsComplete,
								   psSrcSyncInfo->psSyncData->ui32WriteOpsPending,
								   0xFFFFFFFF,
								   1000,
								   1000) != PVRSRV_OK)
			{
				PVR_DPF((PVR_DBG_ERROR, "HardwareMipGen: Timeout failed on waiting for previous texture mipgen"));
			}
		}
							 ...
							 const IMG_UINT32 ui32Yscale)
{
	...
	GLES1_ASSERT (pSrcMap->ui32Width / pDstMap->ui32Width == ui32Xscale);
	GLES1_ASSERT (pSrcMap->ui32Height / pDstMap->ui32Height == ui32Yscale);
	GLES1_ASSERT (ui32Xscale<=2 && ui32Yscale<=2);
	GLES1_ASSERT (ui32Xscale>1 || ui32Yscale>1);
							  ...
							  const IMG_UINT32 ui32Yscale, const TexelFormat *psFormat)
{
	...
	else
	{
		...
		GLES1_ASSERT ((ui32Xscale==2 && ui32Yscale==1) || (ui32Xscale==1 && ui32Yscale==2));
...
************************************************************************************/
static IMG_BOOL SetupMipmapTextureControlWords(GLES1Context *gc, GLESTexture *psTex, IMG_BOOL *pbUnloadTex)
{
    GLESTextureParamState   *psParams = &psTex->sState;
    const GLESTextureFormat *psTexFormat = psTex->psMipLevel[0].psTexFormat;
    IMG_UINT32               ui32Width, ui32Height;	/* PRQA S 3203 */ /* Variables ui32Width and ui32Height are going to be used if SGX_FEATURE_TAG_POT_TWIDDLE is undefined. */
    ...
    *pbUnloadTex = IMG_FALSE;
    ...
    /* HWTQ do NOT support compressed format texture */
    ...
    /* Variables ui32Width and ui32Height are going to be used if SGX_FEATURE_TAG_POT_TWIDDLE is undefined. */
    /* PRQA S 3199 ++ */
    ...
    if(psTex->psMemInfo)
    {
		 ...
		 * If the state word 3 has changed (SGX545) or  
...
************************************************************************************/
IMG_INTERNAL IMG_BOOL HardwareMakeTextureMipmapLevels(GLES1Context *gc, GLESTexture *psTex, IMG_UINT32 ui32TexTarget)
{
	IMG_UINT32               ui32Lod;
	GLESMipMapLevel         *psLevel;
	GLESTextureParamState   *psParams = &psTex->sState;
	...
	IMG_UINT32               ui32OffsetInBytes, ui32BytesPerPixel;
	...
	{
	    case GLES1_TEXTURE_TARGET_2D:
		{
			ui32MaxFace = 1;
			break;
		}
#if defined(GLES1_EXTENSION_TEX_CUBE_MAP)
	    case GLES1_TEXTURE_TARGET_CEM:
		{
			ui32MaxFace = GLES1_TEXTURE_CEM_FACE_MAX;
			break;
		}
#endif
	    default:
	...
	**********************************************************************************************/
	...
	{
		ui32BaseLevel = ui32Face * GLES1_MAX_TEXTURE_MIPMAP_LEVELS;
		...
		if (psLevel->pui8Buffer != GLES1_LOADED_LEVEL)
		{
			...
			if ((psTex->psMemInfo) && (!bUnloadTex))
			{
				...
				FlushAttachableIfNeeded(gc, (GLES1FrameBufferAttachable*)psLevel,
										GLES1_SCHEDULE_HW_WAIT_FOR_3D | GLES1_SCHEDULE_HW_LAST_IN_SCENE);
			}
	...
	for (ui32Face = 0; ui32Face < ui32MaxFace; ui32Face++)
	{
		...
		/* Calculate ui32OffsetInBytes, which is used in CASE 1, 2 & 3 */
...
failed_hwmipgen:
					...
					if (sMemInfo.uAllocSize)
					{
						...
						{
							for (j = 0; j < GLES1_MAX_TEXTURE_MIPMAP_LEVELS; j++)
							{
								ui32Level = (i * GLES1_MAX_TEXTURE_MIPMAP_LEVELS) + j;
		...
		else if (psLevel->pui8Buffer == GLES1_LOADED_LEVEL)
		{
...
/***********************************************************************************
 ...
 Limitations        : We only support power-of-two textures. That's all the spec requires.
************************************************************************************/
IMG_INTERNAL IMG_BOOL MakeTextureMipmapLevels(GLES1Context *gc, GLESTexture *psTex, IMG_UINT32 ui32Face)
{
	IMG_UINT32               ui32Lod;
	GLESMipMapLevel         *psSrcLevel, *psDstLevel, *psLevel;
	const GLESTextureFormat *psFormat;
	...
	/* We don't support mipmapped pbuffers */
	if (psTex->hPBuffer)
	{
		return IMG_FALSE;
	}
	...
	GLES1_ASSERT(ui32Face < GLES1_TEXTURE_CEM_FACE_MAX);

#else /* defined(GLES1_EXTENSION_TEX_CUBE_MAP) */

	GLES1_ASSERT(ui32Face==0);
	...
	for (ui32Lod = ui32BaseLevel+1; ui32Lod < ui32BaseLevel+GLES1_MAX_TEXTURE_MIPMAP_LEVELS; ui32Lod++)
	{
		...
		pui8Dest = TextureCreateLevel(gc, psTex, ui32Lod, psSrcLevel->eRequestedFormat, psSrcLevel->psTexFormat, ui32Width, ui32Height);
		...
		if(psSrcLevel->pui8Buffer == GLES1_LOADED_LEVEL)
		{
			...
			FlushAttachableIfNeeded(gc, (GLES1FrameBufferAttachable*)psSrcLevel,
	...
	psTex->ui32NumLevels = (ui32Lod + 1) % GLES1_MAX_TEXTURE_MIPMAP_LEVELS;
