		...
		if(psSharedState->ui32RefCount == 0)
		{
			if(psSharedState->sBindingSymbolList.uNumBindings)
			{
				for(i=0; i < psSharedState->sBindingSymbolList.uNumBindings; i++)
				{
					...
					GLES2Free(gc, psSymbol->pszName);

					if(psSymbol->uNumBaseTypeMembers)
					{
						for(j=0; j < psSymbol->uNumBaseTypeMembers; j++)
						{
							GLES2Free(gc, psSymbol->psBaseTypeMembers[j].pszName);
						}

						GLES2Free(gc, psSymbol->psBaseTypeMembers);
					}
				}

				GLES2Free(gc, psSharedState->sBindingSymbolList.psBindingSymbolEntries);
			}
		

			if(psSharedState->sBindingSymbolList.uNumCompsUsed)
			{
				GLES2Free(gc, psSharedState->sBindingSymbolList.pfConstantData);
			}
...
IMG_INTERNAL IMG_VOID USESecondaryUploadTaskDelRef(GLES2Context *gc, GLES2USESecondaryUploadTask *psUSESecondaryUploadTask)
{
	...
	if(bDoFree)
	{
		...
		GLES2Free(gc, psUSESecondaryUploadTask);
	}
}
...
static IMG_VOID ShaderIndexableTempsMemDelRef(GLES2Context *gc, GLES2ShaderIndexableTempsMem *psIndexableTempsMem)
{
	...
	if(bDoFree)
	{
		...
		GLES2Free(gc, psIndexableTempsMem);
	}
}
							...
							GLSLBindingSymbolList *psSrcList)
{
	...
	for(i = 0; i < psSrcList->uNumBindings; i++)
	{
		...
		if(psSrcSymbol->uNumBaseTypeMembers)
		{
			...
			{
				GLES2Free(gc, psDstSymbol->pszName);
				goto CleanupSymbols;
			}
			...
			for(j=0; j < psSrcSymbol->uNumBaseTypeMembers; j++)
			{
				...
				if(!psDstBaseSymbol->pszName)
				{
					...
					{
						j--;
						GLES2Free(gc, psDstSymbol->psBaseTypeMembers[j].pszName);
					}
					GLES2Free(gc, psDstSymbol->psBaseTypeMembers);
					GLES2Free(gc, psDstSymbol->pszName);
					goto CleanupSymbols;
				}
...
CleanupSymbols:
	...
	GLES2Free(gc, psDstList->psBindingSymbolEntries);
...
static IMG_BOOL LoadCompilerModule(GLES2Context *gc)
{
	...
	/* Load the module */
	hGLSLCompiler = PVRSRVLoadLibrary(GLES2_GLSL_COMPILER_MODULE_NAME);

	if (!hGLSLCompiler)
	{
		PVR_DPF((PVR_DBG_ERROR, "LoadCompilerModule: Couldn't load library %s", GLES2_GLSL_COMPILER_MODULE_NAME));
		return IMG_FALSE;
	}
	...
	PVRSRVGetLibFuncAddr(hGLSLCompiler,	"GLSLInitCompiler",     (IMG_VOID **)&sFuncTable.pfnInitCompiler);
	PVRSRVGetLibFuncAddr(hGLSLCompiler,	"GLSLCompileToUniflex", (IMG_VOID **)&sFuncTable.pfnCompileToUniflex);
	PVRSRVGetLibFuncAddr(hGLSLCompiler,	"GLSLFreeCompiledUniflexProgram", (IMG_VOID **)&sFuncTable.pfnFreeCompiledUniflexProgram);
	PVRSRVGetLibFuncAddr(hGLSLCompiler,	"GLSLDisplayMetrics", (IMG_VOID **)&sFuncTable.pfnDisplayMetrics);
	PVRSRVGetLibFuncAddr(hGLSLCompiler,	"GLSLShutDownCompiler", (IMG_VOID **)&sFuncTable.pfnShutDownCompiler);
#if defined(GLES2_EXTENSION_GET_PROGRAM_BINARY)
	PVRSRVGetLibFuncAddr(hGLSLCompiler,	"SGXBS_CreateBinaryProgram", (IMG_VOID **)&sFuncTable.pfnCreateBinaryProgram);
#endif 
#if defined(EGL_EXTENSION_ANDROID_BLOB_CACHE)
	PVRSRVGetLibFuncAddr(hGLSLCompiler,	"SGXBS_CreateBinaryShader", (IMG_VOID **)&sFuncTable.pfnCreateBinaryShader);
#endif
...
static IMG_VOID UnloadCompilerModule(GLES2Context *gc)
{
	...
	/* Unload the module */
	if(PVRSRV_OK == PVRSRVUnloadLibrary(gc->sProgram.hGLSLCompiler))
	{
	...
	{
		PVR_DPF((PVR_DBG_ERROR, "UnloadCompilerModule: Unloading the module caused an error"));
	}
}
...
static IMG_VOID ResetProgramLinkedState(GLES2Context *gc, GLES2Program *psProgram)
{
	...
	{
		GLES2Free(gc, psProgram->sVertex.pfConstantData);
		psProgram->sVertex.pfConstantData = IMG_NULL;
	}
	...
	{
		GLES2Free(gc, psProgram->sFragment.pfConstantData);
		psProgram->sFragment.pfConstantData = IMG_NULL;
	}
...
static IMG_BOOL LinkVertexFragmentPrograms(GLES2Context *gc, GLES2Program *psProgram)
{
	...
	*/
	...
	{
		GLES2Free(gc, psProgram->psActiveUniforms);
		psProgram->psActiveUniforms = IMG_NULL;

		GLES2Free(gc, psProgram->ppsActiveUserUniforms);
	...
	{
		GLES2Free(gc, psProgram->psBuiltInUniforms);
	...
	{
		GLES2Free(gc, psProgram->psActiveAttributes);
	...
	{
		GLES2Free(gc, psProgram->psActiveVaryings);
...
bad_alloc:
	...
	GLES2Free(gc, psProgram->pszInfoLog);
...
NoBinary:
	...
	if(psCompiledProgram->bSuccessfullyCompiled)
	{
		...
		{
			GLES2Free(gc, psShader->pszInfoLog);
...
GL_API_EXT void GL_APIENTRY glProgramBinaryOES (GLuint program, GLenum binaryFormat, const void *binary, GLint length)
{
	...
	*/
	...
	{
		psTmp = psBinding;
		GLES2Free(gc, psBinding->pszName);
...
{
	GLES2Free(gc, psShader->pszInfoLog);
	GLES2Free(gc, psShader->pszSource);

	SharedShaderStateDelRef(gc, psShader->psSharedState);

	GLES2Free(gc, psShader);
}
...
IMG_INTERNAL IMG_VOID DestroyUSEShaderVariantGhost(GLES2Context *gc, GLES2USEShaderVariantGhost *psUSEVariantGhost)
{
	...
	while(psPDSVariantGhost)
	{
		...
		GLES2Free(gc, psPDSVariantGhost);

		psPDSVariantGhost = psPDSVariantGhostNext;
	}

	GLES2Free(gc, psUSEVariantGhost);
}
