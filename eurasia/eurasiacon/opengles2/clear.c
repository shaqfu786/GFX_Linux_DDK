...
#define GLES2_CLEAR_VERTEX_FIRSTPHAS_SIZE_IN_BYTES  (ALIGNCOUNT((EURASIA_USE_INSTRUCTION_SIZE * 12), EURASIA_PDS_DOUTU_PHASE_START_ALIGN))
#else
#define GLES2_CLEAR_VERTEX_FIRSTPHAS_SIZE_IN_BYTES  (ALIGNCOUNT((EURASIA_USE_INSTRUCTION_SIZE * 8), EURASIA_PDS_DOUTU_PHASE_START_ALIGN))
#endif

#if defined(SGX_FEATURE_VCB)
/* 1st phase, EMITVCB, PHAS */
#define GLES2_CLEAR_VERTEX_SIZE_IN_BYTES	(GLES2_CLEAR_VERTEX_FIRSTPHAS_SIZE_IN_BYTES + (EURASIA_USE_INSTRUCTION_SIZE * 2))
#else
#define GLES2_CLEAR_VERTEX_SIZE_IN_BYTES	(GLES2_CLEAR_VERTEX_FIRSTPHAS_SIZE_IN_BYTES)
#endif /* defined(SGX_FEATURE_VCB) */
#define GLES2_CLEAR_FRAGMENT_SECTION_SIZE_IN_BYTES	(ALIGNCOUNT((EURASIA_USE_INSTRUCTION_SIZE * 2), EURASIA_PDS_DOUTU_PHASE_START_ALIGN))

#if defined(FIX_HW_BRN_29838)
#define GLES2_CLEAR_FRAGMENT_SIZE_IN_BYTES	((GLES2_CLEAR_FRAGMENT_SECTION_SIZE_IN_BYTES * 16) + (EURASIA_USE_INSTRUCTION_SIZE * 3))
#else
#define GLES2_CLEAR_FRAGMENT_SIZE_IN_BYTES	((GLES2_CLEAR_FRAGMENT_SECTION_SIZE_IN_BYTES * 16) + (EURASIA_USE_INSTRUCTION_SIZE * 2))
#endif /* FIX_HW_BRN_29838 */

#else 
#define GLES2_CLEAR_VERTEX_SIZE_IN_BYTES	(EURASIA_USE_INSTRUCTION_SIZE * 7)
#define GLES2_CLEAR_FRAGMENT_SECTION_SIZE_IN_BYTES	(ALIGNCOUNT(EURASIA_USE_INSTRUCTION_SIZE, EURASIA_PDS_DOUTU_PHASE_START_ALIGN))

#if defined(FIX_HW_BRN_29838)
#define GLES2_CLEAR_FRAGMENT_SIZE_IN_BYTES	((GLES2_CLEAR_FRAGMENT_SECTION_SIZE_IN_BYTES * 16) + (EURASIA_USE_INSTRUCTION_SIZE * 2))
#else
#define GLES2_CLEAR_FRAGMENT_SIZE_IN_BYTES	((GLES2_CLEAR_FRAGMENT_SECTION_SIZE_IN_BYTES * 16) + (EURASIA_USE_INSTRUCTION_SIZE * 1))
#endif /* FIX_HW_BRN_29838 */

#endif /* defined(SGX_FEATURE_USE_UNLIMITED_PHASES) */

/* 16 colormask variants */
...
/***********************************************************************************
 ...
 Outputs            : 
 Returns            : Success
 Description        : Initialises all USSE code blocks required for clears
************************************************************************************/
IMG_INTERNAL IMG_BOOL InitClearUSECodeBlocks(GLES2Context *gc)
{	
	...
	gc->sPrim.psClearVertexCodeBlock = UCH_CodeHeapAllocate(gc->psSharedState->psUSEVertexCodeHeap,
														GLES2_CLEAR_VERTEX_SIZE_IN_BYTES);
    ...
    GLES2_INC_COUNT(GLES2_TIMER_USECODEHEAP_VERT_COUNT, GLES2_CLEAR_VERTEX_SIZE_IN_BYTES >> 2);

	pui32BufferBase = gc->sPrim.psClearVertexCodeBlock->pui32LinAddress;
	
	USEGenWriteSpecialObjVtxProgram (pui32BufferBase, 
	...
	gc->sPrim.psClearFragmentCodeBlock = UCH_CodeHeapAllocate(gc->psSharedState->psUSEFragmentCodeHeap,
														GLES2_CLEAR_FRAGMENT_SIZE_IN_BYTES);
	...
	GLES2_INC_COUNT(GLES2_TIMER_USECODEHEAP_FRAG_COUNT, GLES2_CLEAR_FRAGMENT_SIZE_IN_BYTES >> 2);
	...
	for(ui32ColorMask = 0; ui32ColorMask < GLES2_COLORMASK_ALL; ui32ColorMask++)
	{
		/* Bytes to Dwords conversion */
	    ui32Offset = (GLES2_CLEAR_FRAGMENT_SECTION_SIZE_IN_BYTES * ui32ColorMask) >> 2;
	
#if defined(SGX_FEATURE_USE_UNLIMITED_PHASES)
	    /* Insert Single Phas Instruction */
		BuildPHASLastPhase ((PPVR_USE_INST) &pui32BufferBase[ui32Offset], 0);
		ui32Offset += 2;
#endif /* defined(SGX_FEATURE_USE_UNLIMITED_PHASES) */
	...
	/* Bytes to Dwords conversion */
	ui32Offset = (GLES2_CLEAR_FRAGMENT_SECTION_SIZE_IN_BYTES * GLES2_COLORMASK_ALL) >> 2;
...
************************************************************************************/
IMG_INTERNAL IMG_VOID FreeClearUSECodeBlocks(GLES2Context *gc)
{
	UCH_CodeHeapFree(gc->sPrim.psClearVertexCodeBlock);
...
************************************************************************************/
static IMG_BOOL GetClearSize(GLES2Context *gc, IMG_FLOAT *pfVertices, IMG_BOOL bForceFulLScreen)
{
	...
	if(((gc->ui32Enables & GLES2_SCISSOR_ENABLE) != 0) && !bForceFulLScreen)
	{
...
typedef struct GLES2ClearVertex_TAG
{
	IMG_FLOAT fX;
	IMG_FLOAT fY;
	IMG_FLOAT fZ;
...
************************************************************************************/
static GLES2_MEMERROR SetupVerticesForClear(GLES2Context *gc, IMG_DEV_VIRTADDR *puVertexAddr, 
											IMG_DEV_VIRTADDR *puIndexAddr, IMG_UINT32 *pui32NumIndices,
											IMG_BOOL bForceFullScreen, IMG_FLOAT fDepth)
{
	GLES2ClearVertex *psVertices;
	...
	psVertices = (GLES2ClearVertex *) CBUF_GetBufferSpace(gc->apsBuffers, ui32VertexDWords,
	...
	*puIndexAddr = CBUF_GetBufferDeviceAddress(gc->apsBuffers, (const IMG_VOID *)pui16Indices, CBUF_TYPE_INDEX_DATA_BUFFER);
...
************************************************************************************/
static GLES2_MEMERROR SetupVerticesAndShaderForClear(GLES2Context *gc, IMG_BOOL bForceFullScreen, IMG_FLOAT fDepth)
{
...
************************************************************************************/
static GLES2_MEMERROR SetupFragmentShaderForClear(GLES2Context *gc, IMG_UINT32 *pui32PDSState)
{
	...
	uUSSECodeAddr.uiAddr = gc->sPrim.psClearFragmentCodeBlock->sCodeAddress.uiAddr + 
						  (gc->sState.sRaster.ui32ColorMask * GLES2_CLEAR_FRAGMENT_SECTION_SIZE_IN_BYTES);
	...
	*/
	...
	if (ui32PDSDataSizeDS1 & 7U)
	{
		ui32PDSDataSize = ((ui32PDSDataSizeDS0 + 7U) & ~7U) + ui32PDSDataSizeDS1;
	}

	ui32PDSProgramSize += (ui32PDSDataSize + ((1U << (EURASIA_PDS_DATASIZE_ALIGNSHIFT - 2U)) - 1U)) & ~((1U << (EURASIA_PDS_DATASIZE_ALIGNSHIFT - 2U)) - 1U);
...
************************************************************************************/
IMG_INTERNAL GLES2_MEMERROR SendDepthBiasPrims(GLES2Context *gc)
{
...
************************************************************************************/
IMG_INTERNAL GLES2_MEMERROR SendClearPrims(GLES2Context *gc, IMG_UINT32 ui32ClearFlags, 
										   IMG_BOOL bForceFullScreen, IMG_FLOAT fDepth)
{
	IMG_UINT32 ui32StateHeader;
	...
	*/
	...
	if(ui32ClearFlags & GLES2_CLEARFLAG_COLOR)
	{
	...
	if(ui32ClearFlags & GLES2_CLEARFLAG_STENCIL)
	{
		...
		ui32ISPControlC =	EURASIA_ISPC_SCMP_ALWAYS	|
							...
							(gc->sState.sStencil.ui32FFStencil & ~EURASIA_ISPC_SWMASK_CLRMSK);
	...
	*pui32Buffer++ = 0;
	...
	/*	Texture float info */
	/* ========= */
	*pui32Buffer++ = 0;	/* PRQA S 3199 */ /* pui32Buffer is used in the following GLES_ASSERT. */
...
GL_APICALL void GL_APIENTRY glClear(GLbitfield mask)
{
	...
	if (((mask & GL_COLOR_BUFFER_BIT) != 0)  && gc->sState.sRaster.ui32ColorMask)
	{
		/* Do color clear */
		ui32ClearFlags |= GLES2_CLEARFLAG_COLOR;
	}
	...
	{
		/* Do depth clear */
		ui32ClearFlags |= GLES2_CLEARFLAG_DEPTH;
	}

	if (((mask & GL_STENCIL_BUFFER_BIT) != 0) && gc->psMode->ui32StencilBits)
	{
		/* Do stencil clear */
		ui32ClearFlags |= GLES2_CLEARFLAG_STENCIL;
	}
	...
	if(ui32ClearFlags)
	{
		...
		if(ui32ClearFlags)
		{
			if(SendDrawMaskForClear(gc) != GLES2_NO_ERROR)
			{
			...
			if(SendClearPrims(gc, ui32ClearFlags, IMG_FALSE, gc->sState.sDepth.fClear) != GLES2_NO_ERROR)
			{
				PVRSRVUnlockMutex(gc->psRenderSurface->hMutex);
...
GL_APICALL void GL_APIENTRY glClearColor(GLclampf red, GLclampf green, GLclampf blue, GLclampf alpha)
{
	...
	GLES2_TIME_START(GLES2_TIMES_glClearColor);

	gc->sState.sRaster.sClearColor.fRed   = Clampf(red, GLES2_Zero, GLES2_One);
	gc->sState.sRaster.sClearColor.fGreen = Clampf(green, GLES2_Zero, GLES2_One);
	gc->sState.sRaster.sClearColor.fBlue  = Clampf(blue, GLES2_Zero, GLES2_One);
	gc->sState.sRaster.sClearColor.fAlpha = Clampf(alpha, GLES2_Zero, GLES2_One);

	gc->sState.sRaster.ui32ClearColor = ColorConvertToHWFormat(&gc->sState.sRaster.sClearColor);

	GLES2_TIME_STOP(GLES2_TIMES_glClearColor);
...
GL_APICALL void GL_APIENTRY glClearDepthf(GLclampf depth)
{
	...
	GLES2_TIME_START(GLES2_TIMES_glClearDepthf);

	gc->sState.sDepth.fClear = Clampf(depth, GLES2_Zero, GLES2_One);

	GLES2_TIME_STOP(GLES2_TIMES_glClearDepthf);
}
...
GL_APICALL void GL_APIENTRY glClearStencil(GLint s)
{
	...
	GLES2_TIME_START(GLES2_TIMES_glClearStencil);

	gc->sState.sStencil.ui32Clear = ((IMG_UINT32)s & GLES2_MAX_STENCIL_VALUE);

	GLES2_TIME_STOP(GLES2_TIMES_glClearStencil);
}
