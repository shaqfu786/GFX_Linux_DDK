...
/***********************************************************************************
 ...
 Description        : 
************************************************************************************/
IMG_INTERNAL IMG_UINT32 GetNumStreamDevices(GLES2Context *gc)
{
...
************************************************************************************/
static IMG_VOID SetTextureFormat(GLES2Texture *psTex, GLES2StreamDevice *psBufferDevice)
{
	switch(psBufferDevice->sBufferInfo.pixelformat)
	{
		...
		{
			psTex->psFormat = &TexFormatYUV420;
...
/***********************************************************************************
 ...
 Description        : 
************************************************************************************/
IMG_INTERNAL PVRSRV_ERROR FreeTexStreamState(GLES2Context *gc)
{
	PVRSRV_ERROR eError;
	IMG_UINT32 i;

	GLES2StreamDevice *psBufferDevice = gc->psBufferDevice;
	GLES2StreamDevice *psPreviousDevice;
	...
	while(psBufferDevice)
	{
		...
		{
			GLES2Free(gc, psBufferDevice->psBuffer);
		}
		...
		GLES2Free(gc, psPreviousDevice->psExtTexState);

		GLES2Free(gc, psPreviousDevice);
	}
...
/***********************************************************************************
 ...
 Description        : 
************************************************************************************/
static IMG_BOOL FindBufferDevice(GLES2Context *gc, IMG_UINT32 device, GLES2StreamDevice **ppsBufferDevice)
{
	GLES2StreamDevice *psBufferDevice = gc->psBufferDevice;
...
static PVRSRV_ERROR CreateTextureStreamState(GLES2StreamDevice *psBufferDevice)
{
...
/***********************************************************************************
 ...
 Description        : 
************************************************************************************/
static IMG_BOOL AddBufferDevice(GLES2Context *gc, IMG_UINT32 device, GLES2StreamDevice **ppsBufferDevice)
{
	...
	psBufferDevice = GLES2Calloc(gc, sizeof(GLES2StreamDevice));
	...
	psBufferDevice->psExtTexState = GLES2Calloc(gc, sizeof(GLES2ExternalTexState));
	...
	pui32DeviceID = GLES2Calloc(gc, sizeof(IMG_UINT32)*ui32DevCount);
	...
	psBufferDevice->psBuffer = GLES2Calloc(gc, sizeof(GLES2DeviceBuffer) * psBufferDevice->sBufferInfo.ui32BufferCount);
...
ErrorCase:
	...
	if (psBufferDevice->psExtTexState)
	{
		GLES2Free(gc, psBufferDevice->psExtTexState);
		psBufferDevice->psExtTexState = IMG_NULL;
	}

	GLES2Free(gc, psBufferDevice);
...
GL_API_EXT void GL_APIENTRY glGetTexStreamDeviceAttributeivIMG(GLint device, GLenum pname, GLint *params)
{
	...
	GLES2_TIME_START(GLES2_TIMES_glGetTexStreamDeviceAttributeiv);
	...
	if(!FindBufferDevice(gc, (IMG_UINT32)device, &psBufferDevice))
	{
		...
		if(!AddBufferDevice(gc, (IMG_UINT32)device, &psBufferDevice))
		{
			...
			GLES2_TIME_STOP(GLES2_TIMES_glGetTexStreamDeviceAttributeiv);
...
GL_API_EXT void GL_APIENTRY glTexBindStreamIMG(GLint device, GLint deviceoffset)
{
	...
	GLES2_TIME_START(GLES2_TIMES_glTexBindStream);
...
bad_val:
			...
			GLES2_TIME_STOP(GLES2_TIMES_glTexBindStream);
	...
	psTex = gc->sTexture.apsBoundTexture[gc->sState.sTexture.ui32ActiveTexture][GLES2_TEXTURE_TARGET_STREAM];
	...
	if(psTex->psBufferDevice != psBufferDevice || psTex->ui32BufferOffset != (IMG_UINT32)deviceoffset)
	{
		...
		/* Setup texture mipmap level[0]'s width and height */
...
GL_API_EXT const GLubyte * GL_APIENTRY glGetTexStreamDeviceNameIMG(GLint device)
{
	...
	GLES2_TIME_START(GLES2_TIMES_glGetTexStreamDeviceName);
	...
	if(!FindBufferDevice(gc, (IMG_UINT32)device, &psBufferDevice))
	{
		...
		if(!AddBufferDevice(gc, (IMG_UINT32)device, &psBufferDevice))
		{
			...
			SetError(gc, GL_INVALID_VALUE);
			GLES2_TIME_STOP(GLES2_TIMES_glGetTexStreamDeviceName);
