										...
										SGX_QUEUETRANSFER *psQueueTransfer)
{
	...
	eResult = SGXQueueTransfer(gc->psSysContext->hTransferContext, psQueueTransfer);
	...
	if(eResult != PVRSRV_OK)
	{
		...
		{		
			...
			if(PVRSRVPollForValue( gc->psSysContext->psConnection,
								   ...
								   &psDstSyncInfo->psSyncData->ui32WriteOpsComplete, 
								   psDstSyncInfo->psSyncData->ui32WriteOpsPending, 
								   0xFFFFFFFF,
								   1000, 
								   1000) != PVRSRV_OK)
			{
				PVR_DPF((PVR_DBG_ERROR, "HWTQTextureUpload: Timeout failed on waiting for previous texture transfer"));
			}
		}
	...
	if(eResult != PVRSRV_OK)
	{
		...
		{		
			...
			if(PVRSRVPollForValue( gc->psSysContext->psConnection,
								   ...
								   1000) != PVRSRV_OK)
			{
				PVR_DPF((PVR_DBG_ERROR, "HWTQTextureNormalBlit: Timeout failed on waiting for previous texture transfer"));
			}
		}
	...
	if(eResult != PVRSRV_OK)
	{
		...
		{		
			...
			if(PVRSRVPollForValue( gc->psSysContext->psConnection,
								   ...
								   1000) != PVRSRV_OK)
			{
				PVR_DPF((PVR_DBG_ERROR, "HWTQTextureBufferBlit: Timeout failed on waiting for previous texture transfer"));
			}
		}
											...
											IMG_UINT32 ui32Lod, IMG_VOID *pvBuffer)
{
	...
	{		
		...
		if(PVRSRVPollForValue(gc->psSysContext->psConnection,
						...
						1000) != PVRSRV_OK)
		{
			PVR_DPF((PVR_DBG_ERROR, "ReadBackTextureData: Timeout failed on waiting for texture upload"));
			return;
		}
	}
									  ...
									  IMG_UINT32              ui32SizeInBytes)
{
    ...
    /* IMG_BOOL bHWTextureBufferBlitted = IMG_FALSE; */
    ...
    if (!gc->sAppHints.bDisableHWTQBufferBlit) 
	{
										 ...
										 ui32SizeInBytes, &sQueueTransfer))
		{
			...
			{
			    /* bHWTextureBufferBlitted = IMG_TRUE; */
