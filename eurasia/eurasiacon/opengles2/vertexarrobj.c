...
************************************************************************************/
IMG_INTERNAL IMG_BOOL CreateVertexArrayObjectState(GLES2Context *gc)
{
	...
	gc->sVAOMachine.sDefaultVAO.ui32DirtyState               = GLES2_DIRTYFLAG_VAO_ALL;
...
************************************************************************************/
static IMG_BOOL WaitUntilVAONotUsed(GLES2Context *gc, GLES2VertexArrayObject *psVAO)
{
	/*
...
static IMG_VOID FreeVertexArrayObjectInternalPointers(GLES2Context *gc, GLES2VertexArrayObject *psVAO)
{
	...
	{
	    GLES2Free(gc, psVAO->psPDSVertexState);
	...
	{
	    GLES2Free(gc, psVAO->psPDSVertexShaderProgram);
...
/***********************************************************************************
 ...
 Description        : Unbinds all vertex array objects, and deletes this also.
************************************************************************************/
IMG_INTERNAL IMG_VOID FreeVertexArrayObjectState(GLES2Context *gc)
{
    GLES2VertexArrayObject *psVAO = gc->sVAOMachine.psActiveVAO;
	GLES2VertexArrayObject *psDefaultVAO = &(gc->sVAOMachine.sDefaultVAO); 
	...
	if (!VAO_IS_ZERO(gc))
	{
		GLES2NamesArray  *psNamesArray = gc->apsNamesArray[GLES2_NAMETYPE_VERARROBJ - GLES2_MAX_SHAREABLE_NAMETYPE];

		GLES_ASSERT(psNamesArray);

		/* Unbind the current named VAO from the context by decreasing its RefCount by 1 */
		NamedItemDelRef(gc, psNamesArray, (GLES2NamedItem*)psVAO);
...
************************************************************************************/
static IMG_VOID FreeVertexArrayObject(GLES2Context *gc, GLES2VertexArrayObject *psVAO, IMG_BOOL bIsShutdown)
{
...
************************************************************************************/
static IMG_VOID DisposeVertexArrayObject(GLES2Context *gc, GLES2NamedItem *psItem, IMG_BOOL bIsShutdown)
{
...
************************************************************************************/
IMG_INTERNAL IMG_VOID SetupVertexArrayObjectNameArray(GLES2NamesArray *psNamesArray)
{
...
************************************************************************************/
static GLES2VertexArrayObject* CreateVertexArrayObject(GLES2Context *gc, IMG_UINT32 ui32Name)
{
    GLES2VertexArrayObject *psVAO;
	
	PVR_UNREFERENCED_PARAMETER(gc);

	psVAO = (GLES2VertexArrayObject *) GLES2Calloc(gc, sizeof(GLES2VertexArrayObject));
	...
	return psVAO; /* if GLES2Calloc failed, then return NULL */
}
...
GL_API_EXT void GL_APIENTRY glBindVertexArrayOES(GLuint vertexarray)
{
	...
	GLES2VertexArrayObject *psCurrentVAO;
	GLES2VertexArrayObject *psVAO;
	...
	psNamesArray = gc->apsNamesArray[GLES2_NAMETYPE_VERARROBJ - GLES2_MAX_SHAREABLE_NAMETYPE];
	...
	if (vertexarray) 
	{
		...
		psVAO = (GLES2VertexArrayObject *) NamedItemAddRef(psNamesArray, vertexarray);
		...
		if (!psVAO)
		{
			...
			GLES_ASSERT(psVAO->sNamedItem.ui32Name == vertexarray);
			
			if(!InsertNamedItem(psNamesArray, (GLES2NamedItem*)psVAO))
			{
			    /* ((GLES2NamedItem*)psVAO != IMG_NULL) and
				  (((GLES2NamedItem*)psVAO->ui32Name) != IMG_NULL) 
	...
	{
		if (psCurrentVAO && (((GLES2NamedItem*)psCurrentVAO)->ui32Name != 0))
		{
			NamedItemDelRef(gc, psNamesArray, (GLES2NamedItem*)psCurrentVAO);
		}

		/* Bind the new VAO to this context */
		gc->sVAOMachine.psActiveVAO = psVAO;

		/* Set the dirty binding flag for VAO */
		psVAO->ui32DirtyState |= GLES2_DIRTYFLAG_VAO_BINDING;		
...
GL_API_EXT void GL_APIENTRY glDeleteVertexArraysOES(GLsizei n, const GLuint *vertexarrays)
{
	...
	GLES2_TIME_START(GLES2_TIMES_glDeleteVertexArraysOES);

	if (n < 0)
	{
		SetError(gc, GL_INVALID_VALUE);
		GLES2_TIME_STOP(GLES2_TIMES_glDeleteVertexArraysOES);
		return;
	}
	else if (n == 0)
	{
...
GL_API_EXT void GL_APIENTRY glGenVertexArraysOES(GLsizei n, GLuint *vertexarrays)
{
	...
	GLES2_TIME_START(GLES2_TIMES_glGenVertexArraysOES);

	if (n < 0)
	{
		SetError(gc, GL_INVALID_VALUE);
		GLES2_TIME_STOP(GLES2_TIMES_glGenVertexArraysOES);
		return;
	}
	else if (n == 0)
	{
	...
	GLES_ASSERT(gc->apsNamesArray[GLES2_NAMETYPE_VERARROBJ - GLES2_MAX_SHAREABLE_NAMETYPE] != IMG_NULL);

	NamesArrayGenNames(gc->apsNamesArray[GLES2_NAMETYPE_VERARROBJ - GLES2_MAX_SHAREABLE_NAMETYPE], 
					   (IMG_UINT32)n, 
...
GL_API_EXT GLboolean GL_APIENTRY glIsVertexArrayOES(GLuint vertexarray)
{
	...
	GLES2_TIME_START(GLES2_TIMES_glIsVertexArrayOES);

	if (vertexarray == 0)
	{
		GLES2_TIME_STOP(GLES2_TIMES_glIsVertexArrayOES);
		return GL_FALSE;
	}
