...
IMG_INTERNAL IMG_VOID WaitForTA(GLES2Context *gc)
{
	...
	if(PVRSRVPollForValue(	gc->psSysContext->psConnection,
						gc->psSysContext->sHWInfo.sMiscInfo.hOSGlobalEvent,
						pui32ReadOffset,
						gc->apsBuffers[CBUF_TYPE_VDM_CTRL_BUFFER]->ui32CommittedHWOffsetInBytes,
						0xFFFFFFFF,
						1000,
						GLES2_DEFAULT_WAIT_RETRIES) != PVRSRV_OK)
	{
//SUPPORT_FORCE_FLIP_WORKAROUND
		PVRSRV_MISC_INFO sMiscInfo;

		PVR_DPF((PVR_DBG_ERROR, "WaitUntilBufObjNotUsed: Timeout failed on waiting for TA"));

		sMiscInfo.ui32StateRequest = PVRSRV_MISC_INFO_FORCE_SWAP_TO_SYSTEM_PRESENT;
		PVRSRVGetMiscInfo(gc->ps3DDevData->psConnection, &sMiscInfo);

		/* FIXME: Should use ClientEvent instead of SWAP_TO_SYSTEM_PRESENT
		 * but this requires some reworking of PVRSRVPollForValue. For now,
		 * assume if the SWAP_TO_SYSTEM_PRESENT doesn't work, the debug
		 * info will get printed later on by another timeout loop.
		 */
		//PVRSRVClientEvent(PVRSRV_CLIENT_EVENT_HWTIMEOUT, gc->ps3DDevData, IMG_NULL);
...
/***********************************************************************************
 Function Name      : WaitForRender
 Inputs             : psRenderSurfaceSyncInfo
 Outputs            : -
 Returns            : -
...
static IMG_VOID WaitForRender(GLES2Context *gc, PVRSRV_CLIENT_SYNC_INFO *psRenderSurfaceSyncInfo)
{
	...
	GLES2_TIME_START(GLES2_TIMER_WAITING_FOR_3D_TIME);

	while(SYNCINFO_WRITEOPS_COMPLETE(psRenderSurfaceSyncInfo) < SYNCINFO_WRITEOPS_PENDING(psRenderSurfaceSyncInfo))
	{
//SUPPORT_FORCE_FLIP_WORKAROUND
		if(ui32TriesLeft == (GLES2_DEFAULT_WAIT_RETRIES / 2))
		{
			PVRSRV_MISC_INFO sMiscInfo;

			PVR_DPF((PVR_DBG_ERROR, "WaitForRender: Trying force-flip workaround"));
...
//SUPPORT_FORCE_FLIP_WORKAROUND
		...
		if(PVRSRVEventObjectWait(gc->ps3DDevData->psConnection,
								 gc->psSysContext->sHWInfo.sMiscInfo.hOSGlobalEvent) != PVRSRV_OK)
		{
...
/***********************************************************************************
 ...
 Inputs             : gc, psRenderSurface, ui32ClearFlags
 Outputs            : -
 Returns            : -
 Description        : Sets up the force/enable depth/stencil load/store flags
 	 	 	 	 	  in the ZLS control register
************************************************************************************/
static IMG_VOID SetupZLSControlReg(GLES2Context *gc, EGLRenderSurface *psRenderSurface, IMG_BOOL bOverflowRender, IMG_UINT32 ui32ClearFlags)
{
		...
		switch(gc->sAppHints.ui32ExternalZBufferMode)
		{
			...
			case EXTERNAL_ZBUFFER_MODE_ALLOC_ONDEMAND_USED_ALWAYS:
			{
				...
				if(gc->sFrameBuffer.psActiveFrameBuffer == &gc->sFrameBuffer.sDefaultFrameBuffer)
				{
					...
					if(((gc->ui32Enables &  GLES2_SCISSOR_ENABLE) != 0) && !gc->bFullScreenScissor)
					{
					...
					{
						if((ui32ClearFlags & GLES2_CLEARFLAG_DEPTH) == 0)
						{
							ui32ISPZLSControl |= EUR_CR_ISP_ZLSCTL_ZLOADEN_MASK | EUR_CR_ISP_ZLSCTL_FORCEZLOAD_MASK;
						}

						if((ui32ClearFlags & GLES2_CLEARFLAG_STENCIL) == 0)
						{
			...
			case EXTERNAL_ZBUFFER_MODE_ALLOC_NEVER_USED_NEVER:
			{
				...
				GLES_ASSERT(gc->sAppHints.ui32ExternalZBufferMode!=EXTERNAL_ZBUFFER_MODE_ALLOC_NEVER_USED_NEVER);
										  ...
										  IMG_UINT32 ui32RoundedHeight)
{
	...
	GLES_ASSERT(psRenderSurface->hEGLSurface && psRenderSurface->psZSBufferMemInfo);
...
#if defined (SGX545)
	/* Set OGLES sampling (0.5,0.5) for thin lines */
...
************************************************************************************/
static IMG_BOOL ValidateMemory(GLES2Context *gc)
{
...
************************************************************************************/
static IMG_BOOL InsertItemIntoFlushList(GLES2Context *gc, EGLRenderSurface *psRenderSurface, GLES2Texture *psTex)
{
	GLES2SurfaceFlushList *psItem;

	psItem = GLES2Malloc(gc, sizeof(GLES2SurfaceFlushList));
...
************************************************************************************/
static IMG_BOOL StartFrame(GLES2Context *gc, IMG_UINT32 *pui32ClearFlags, PVRSRV_CLIENT_SYNC_INFO *psAccumSyncInfo)
{
	 ...
	 */ 
	if(SetupPixelEventProgram(gc, &psRenderSurface->sPixelBEState, IMG_FALSE) != GLES2_NO_ERROR)
	{
		return IMG_FALSE;
	}
	 ...
	 * so we should not be able to fail this call as we are at the start of a frame. 
	 */ 
	if(SetupBGObject(gc, IMG_FALSE, psRenderSurface->aui32HWBGObjPDSState) != GLES2_NO_ERROR)
	{
		return IMG_FALSE;
	}
	...
	*/
	...
	if(psRenderSurface->bMultiSample)
	{
		psRenderSurface->ui32TriangleSplitPixelThreshold = 0;
	}
	else
	{
		psRenderSurface->ui32TriangleSplitPixelThreshold = EURASIA_DEFAULT_TAG_TRIANGLE_SPLIT * 2;
	}
#endif
	 ...
	 */
	if(!(ps3DRegs->sISPZLSControl.ui32RegVal & EUR_CR_ISP_ZLSCTL_FORCEZLOAD_MASK) &&
	  (!(*pui32ClearFlags & GLES2_CLEARFLAG_DEPTH) || ((gc->ui32Enables & GLES2_SCISSOR_ENABLE) && !gc->bFullScreenScissor)))
	{
		if(SendClearPrims(gc, GLES2_CLEARFLAG_DEPTH, IMG_TRUE, 1.0f) != GLES2_NO_ERROR)
		{
			return IMG_FALSE;
		}		
	...
	if(((*pui32ClearFlags & GLES2_CLEARFLAG_COLOR) == 0) || (((gc->ui32Enables & GLES2_SCISSOR_ENABLE) != 0) && !gc->bFullScreenScissor))
	{
			...
			gc->psDrawParams->ui32AccumHWAddress != gc->psDrawParams->ui32HWSurfaceAddress)
		{
			...
			if( (((gc->ui32Enables & GLES2_SCISSOR_ENABLE) != 0) && !gc->bFullScreenScissor) ||
				((*pui32ClearFlags & GLES2_CLEARFLAG_DEPTH) == 0))
			{
			...
			else
			{
				...
				*pui32ClearFlags &= ~GLES2_CLEARFLAG_DEPTH;
			 ...
			 * so we should not be able to fail this call as we are at the start of a frame. 
			 */ 
			if(SendAccumulateObject(gc, bFullScreenDepthClear, gc->sState.sDepth.fClear) != GLES2_NO_ERROR)
			{
				return IMG_FALSE;
			}		
			...
			if(psAccumSyncInfo)
			{
				GLES_ASSERT(psRenderSurface->ui32NumSrcSyncs == 0);
	...
	if(psRenderSurface->hEGLSurface == IMG_NULL)
	{
		...
		{
			psColourTex = ((GLES2MipMapLevel *)psColorAttachment)->psTex;

			if(!InsertItemIntoFlushList(gc, psRenderSurface, psColourTex))
			{
				return IMG_FALSE;
			}
		}
		...
		{
			psDepthTex = ((GLES2MipMapLevel *)psDepthAttachment)->psTex;

			if(!InsertItemIntoFlushList(gc, psRenderSurface, psDepthTex))
			{
				return IMG_FALSE;
			}
		}
		...
		if(psStencilAttachment && (psStencilAttachment->eAttachmentType==GL_TEXTURE))
		{
			psStencilTex = ((GLES2MipMapLevel *)psStencilAttachment)->psTex;

			if(psStencilTex!=psDepthTex)
			{
				if(!InsertItemIntoFlushList(gc, psRenderSurface, psStencilTex))
				{
					return IMG_FALSE;
				}
			}
...
#endif /* defined(GLES2_EXTENSION_PACKED_DEPTH_STENCIL) */
		{
			psStencilTex = IMG_NULL;
		}

		/* No texture attachments, so just attach the render surface */
		if((psColourTex==IMG_NULL) && (psDepthTex==IMG_NULL) && (psStencilTex==IMG_NULL))
		{
			if(!InsertItemIntoFlushList(gc, psRenderSurface, IMG_NULL))
			{
				return IMG_FALSE;
			}
		}
...
IMG_INTERNAL IMG_BOOL PrepareToDraw(GLES2Context *gc, IMG_UINT32 *pui32ClearFlags, IMG_BOOL bTakeLock)
{
...
#if defined(SGX_FEATURE_DEPTH_BIAS_OBJECTS)
	IMG_BOOL bClear = (*pui32ClearFlags) ? IMG_TRUE : IMG_FALSE;
#endif

	GLES2_TIME_START(GLES2_TIMER_PREPARE_TO_DRAW_TIME);
	...
	if(gc->psRenderSurface && gc->psRenderSurface->bInFrame)
	{
		...
		{
			if(SendDrawMaskForPrimitive(gc) != GLES2_NO_ERROR)
			{
				SetError(gc, GL_OUT_OF_MEMORY);
				PVRSRVUnlockMutex(gc->psRenderSurface->hMutex);
				GLES2_TIME_STOP(GLES2_TIMER_PREPARE_TO_DRAW_TIME);
				return IMG_FALSE;
			}
		}

#if defined(SGX_FEATURE_DEPTH_BIAS_OBJECTS)
		if(!bClear && ((gc->ui32Enables & GLES2_POLYOFFSET_ENABLE) != 0))
		{
			...
			else if(gc->psRenderSurface->ui32DepthBiasState == EGL_RENDER_DEPTHBIAS_REGISTER_USED)
			{
				...
				{
					if(SendDepthBiasPrims(gc) != GLES2_NO_ERROR)
					{
						SetError(gc, GL_OUT_OF_MEMORY);
						PVRSRVUnlockMutex(gc->psRenderSurface->hMutex);
	...
	else
	{
		...
		if(gc->sFrameBuffer.psActiveFrameBuffer == &gc->sFrameBuffer.sDefaultFrameBuffer)
		{
			   ...
			   !gc->psRenderSurface->bInExternalFrame)
			{
				...
				{
					PVR_DPF((PVR_DBG_ERROR,"PrepareToDraw: Invalid drawable"));
				...
				GLES_ASSERT(sParams.psRenderSurface);
				    ...
				    bCreateZSBuffer)
				{
					...
					if(!KEGLResizeRenderSurface(gc->psSysContext, 
												&sParams, 
												gc->psMode->ui32AntiAliasMode ? IMG_TRUE : IMG_FALSE, 
												bCreateZSBuffer,
												gc->psRenderSurface))
					{
		...
		/* Drawmask inits to all on */
		if ((gc->ui32Enables & GLES2_SCISSOR_ENABLE) ||
...
************************************************************************************/
static IMG_VOID PDumpBufferObject(GLES2Context *gc, const IMG_VOID* pvFunctionContext, GLES2NamedItem *psItem)
{
	GLES2BufferObject *psBufferObject = (GLES2BufferObject*)psItem;

	PVR_UNREFERENCED_PARAMETER(pvFunctionContext);

	GLES_ASSERT(psBufferObject);
...
/***********************************************************************************
 ...
 Description        : PDumps a buffer object. Used by ScheduleTA()
************************************************************************************/
static IMG_VOID PDumpVertexArrayObject(GLES2Context *gc, const IMG_VOID* pvFunctionContext, GLES2NamedItem *psItem)
{
	GLES2VertexArrayObject *psVAO = (GLES2VertexArrayObject*)psItem;
...
************************************************************************************/
static IMG_VOID PDumpTerminate(GLES2Context *gc, EGLTerminateState *psTerminateState)
{
...
************************************************************************************/
static IMG_VOID PDumpShaders(GLES2Context *gc)
{
	IMG_UINT32         i, ui32Offset;
	UCH_UseCodeBlock  *psCodeBlock;
	UCH_UseCodeHeap   *apsHeap[3];
	...
	if(gc->sPrim.sBRN26922State.bDump)
	{
		if(gc->sPrim.sBRN26922State.psBRN26922MemInfo)
		{
			PDUMP_STRING((gc, "Dumping BRN_26922 Dummy single pixel FB address 0x%x\r\n",
									gc->sPrim.sBRN26922State.psBRN26922MemInfo->sDevVAddr.uiAddr ));		
			PDUMP_MEM(gc, gc->sPrim.sBRN26922State.psBRN26922MemInfo, 0, gc->sPrim.sBRN26922State.psBRN26922MemInfo->uAllocSize);			
		}

		gc->sPrim.sBRN26922State.bDump = IMG_FALSE;
	}
...
************************************************************************************/
static IMG_VOID PDumpTextures(GLES2Context *gc)
{
	...
	while(psResource)
	{
		...
		psTex = (GLES2Texture*)((IMG_UINTPTR_T)psResource -offsetof(GLES2Texture, sResource));
	...
	if(PDUMP_ISCAPTURING(gc))
	{
		...
		{
			/* Unavoidable tricky pointer arithmetic */
			psGhost = (GLES2Ghost*)((IMG_UINTPTR_T)psResource -offsetof(GLES2Ghost, sResource));
...
IMG_INTERNAL IMG_VOID KickUnFlushed_ScheduleTA(IMG_VOID *pvContext, IMG_VOID *pvRenderSurface)
{
	...
	if (pvRenderSurface)
	{
		...
		{
			ui32Flags |= GLES2_SCHEDULE_HW_MIDSCENE_RENDER;
		}
	}
	else
	{
...
IMG_INTERNAL IMG_VOID KickLimit_ScheduleTA(IMG_VOID *pvContext, IMG_BOOL bLastInScene)
{
	...
	if (bLastInScene)
	{
		ui32Flags = GLES2_SCHEDULE_HW_LAST_IN_SCENE | GLES2_SCHEDULE_HW_MIDSCENE_RENDER;
	}
...
************************************************************************************/
static IMG_EGLERROR DoKickTA(GLES2Context *gc, EGLRenderSurface *psRenderSurface, IMG_UINT32 ui32KickFlags)
{
	...
	if(ui32KickFlags & GLES2_SCHEDULE_HW_MIDSCENE_RENDER)
	{
	...
	if(psRenderSurface->bFirstKick)
	{
		...
		if(psRenderSurface->hEGLSurface)
		{
			...
			/* If this is an EGL surface, it must be the currently bound surface */
			GLES_ASSERT(gc->psRenderSurface == psRenderSurface);

			/* If this is an EGL surface, we can check the default framebuffer, even if this context is
			 * not the context the surface is currently bound to, as it must be compatible
			 */
			if(gc->sFrameBuffer.sDefaultFrameBuffer.sMode.ui32AntiAliasMode)
			{
			...
			if(bMultiSampleChanged)
			{
				if(!KEGLResizeRenderSurface(gc->psSysContext, 
											...
											psRenderSurface))
				{
					PVR_DPF((PVR_DBG_ERROR,"DoKickTA: KEGLResizeRenderSurface() failed"));
		...
		{
			if(SetupPixelEventProgram(gc, &psRenderSurface->sPixelBEState, IMG_TRUE) != GLES2_NO_ERROR)
			{
				PVR_DPF((PVR_DBG_ERROR,"DoKickTA: SetupPixelEventProgram() failed"));
	   ...
	   (gc->sAppHints.ui32ExternalZBufferMode!=EXTERNAL_ZBUFFER_MODE_ALLOC_NEVER_USED_NEVER))
	{
		if(!psRenderSurface->psZSBufferMemInfo)
		{
			...
			if (GLES2ALLOCDEVICEMEM(&gc->psSysContext->s3D,
	...
	if(ui32KickFlags & GLES2_SCHEDULE_HW_LAST_IN_SCENE)
	{
		ui32Flags |= SGX_KICKTA_FLAGS_TERMINATE;

		GLES2_INC_COUNT(GLES2_TIMER_KICK_3D, 1);
	}
	...
	/* Switch fragment buffer to render surface being rendered (not currently bound one) */
	...
	if(psRenderSurface->bMultiSample)
	{
		...
		GLES_ASSERT(psKickTA->hRTDataSet);
	}
	else
	{
	...
	psKickTA->sKickTACommon.bSeperateStencilLoadStore = IMG_TRUE;
...
#if defined(EUR_CR_PDS_PP_INDEPENDANT_STATE)
	if(gc->sAppHints.bDynamicSplitCalc)
	{
		psKickTA->sKickTACommon.ui32TriangleSplitPixelThreshold = psRenderSurface->ui32TriangleSplitPixelThreshold;
	}
	else
	{
		psKickTA->sKickTACommon.ui32TriangleSplitPixelThreshold = gc->sAppHints.ui32TriangleSplitPixelThreshold;
	}
#endif
	
	if(OutputTerminateState(gc, psRenderSurface, ui32KickFlags) != GLES2_NO_ERROR)
	{
	...
	GLES_ASSERT(psKickTA->sKickTACommon.ui32NumTAStatusVals<SGX_MAX_TA_STATUS_VALS);

	/* Point the next TA status update to the TA KRM device memory */
	...
	if(PDUMP_ISCAPTURING(gc))
	{
		...
		NamesArrayMapFunction(gc, gc->psSharedState->apsNamesArray[GLES2_NAMETYPE_BUFOBJ], PDumpBufferObject, IMG_NULL);

#if defined(GLES2_EXTENSION_VERTEX_ARRAY_OBJECT)
		/* Dump all vertex array  objects */
		NamesArrayMapFunction(gc, gc->apsNamesArray[GLES2_NAMETYPE_VERARROBJ - GLES2_MAX_SHAREABLE_NAMETYPE], PDumpVertexArrayObject, IMG_NULL);
#endif
		...
		if(!gc->bPrevFrameDumped)
		{
			...
			PDUMP_STRING((gc, "Dumping Black white texture at device virtual address 0x%x\r\n",
						gc->psSharedState->psTextureManager->psBlackDummyTexture->sDevVAddr.uiAddr));
			PDUMP_MEM(gc, gc->psSharedState->psTextureManager->psBlackDummyTexture, 0, 
						gc->psSharedState->psTextureManager->psBlackDummyTexture->uAllocSize);
		...
		for(i=0; i < CBUF_NUM_BUFFERS; i++)
		{
			...
			if(psBuffer)
			{
				...
				sDumpBuffer[sKickTAPDump.ui32BufferArraySize].pszName = (IMG_CHAR *)((IMG_UINTPTR_T)pszBufferNames[i]);	/* PRQA S 0311 */
		...
		*/
		...
		if(psRenderSurface->eColourSpace!=PVRSRV_COLOURSPACE_FORMAT_UNKNOWN)
		{
#if defined(SGX_FEATURE_UNIFIED_STORE_64BITS)
			...
			switch(ui32Temp)
			{
				case EURASIA_PIXELBE_MEMLAYOUT_TWIDDLED:
				{
					sDumpBitmap.ui32Flags = SGX_KICKTA_DUMPBITMAP_FLAGS_TWIDDLED;

					break;
				}
				case EURASIA_PIXELBE_MEMLAYOUT_TILED:
				{
					sDumpBitmap.ui32Flags = SGX_KICKTA_DUMPBITMAP_FLAGS_TILED;

					break;
				}
				default:
				case EURASIA_PIXELBE_MEMLAYOUT_LINEAR:
				{
	...
	if(psRenderSurface->eColourSpace!=PVRSRV_COLOURSPACE_FORMAT_UNKNOWN)
	{
		ui32Flags |= SGX_KICKTA_FLAGS_COLOURALPHAVALID;
	}

	if(ui32KickFlags & GLES2_SCHEDULE_HW_BBOX_RENDER)
	{
	...
	else
	{
		...
		GLES_ASSERT(psKickTA->sKickTACommon.ui32Num3DStatusVals<SGX_MAX_3D_STATUS_VALS);
		...
		if(ui32KickFlags & (GLES2_SCHEDULE_HW_LAST_IN_SCENE|GLES2_SCHEDULE_HW_DISCARD_SCENE))
		{
			psRenderSurface->sRenderStatusUpdate.ui32StatusValue++;
		}

		if(ui32KickFlags & GLES2_SCHEDULE_HW_POST_BBOX_RENDER)
		{
	...
	GLES2_TIME_STOP(GLES2_TIMER_SGXKICKTA_TIME);

	if(eError!=PVRSRV_OK)
	{
		if((ui32KickFlags & GLES2_SCHEDULE_HW_BBOX_RENDER) == 0)
		{
	...
	CBUF_UpdateBufferCommittedHWOffsets(gc->apsBuffers, 
	                                    (ui32KickFlags & (GLES2_SCHEDULE_HW_LAST_IN_SCENE | GLES2_SCHEDULE_HW_DISCARD_SCENE)) ? IMG_TRUE : IMG_FALSE);
	...
	if((ui32KickFlags & (GLES2_SCHEDULE_HW_LAST_IN_SCENE|GLES2_SCHEDULE_HW_BBOX_RENDER)) == GLES2_SCHEDULE_HW_LAST_IN_SCENE)
	{
		psRenderSurface->bFirstKick = IMG_TRUE;
		...
		if((ui32KickFlags & GLES2_SCHEDULE_HW_IGNORE_FLUSHLIST) == 0)
		{
			if(psRenderSurface->hEGLSurface == IMG_NULL)
			{
				IMG_BOOL bFound = IMG_FALSE;
				GLES2SurfaceFlushList **ppsFlushList;
	...
	gc->ui32EmitMask |=	(GLES2_EMITSTATE_MTE_STATE_OUTPUTSELECTS |
						 ...
						 GLES2_EMITSTATE_MTE_STATE_REGION_CLIP |
...
************************************************************************************/
IMG_INTERNAL IMG_EGLERROR ScheduleTA(GLES2Context *gc, EGLRenderSurface *psRenderSurface, IMG_UINT32 ui32KickFlags)
{
	IMG_EGLERROR eError;
	...
	if(psRenderSurface->bInFrame)
	{
	    if(psRenderSurface->bPrimitivesSinceLastTA           || /* TA (not 3D) can be kicked even though hasn't reached to the last primitive in the scene yet */
