...
#define GLES2_SCISSOR_VERTEX_FIRSTPHAS_SIZE_IN_BYTES (ALIGNCOUNT((EURASIA_USE_INSTRUCTION_SIZE * 9), EURASIA_PDS_DOUTU_PHASE_START_ALIGN))
#else
#define GLES2_SCISSOR_VERTEX_FIRSTPHAS_SIZE_IN_BYTES (ALIGNCOUNT((EURASIA_USE_INSTRUCTION_SIZE * 5), EURASIA_PDS_DOUTU_PHASE_START_ALIGN))
#endif

#if defined(SGX_FEATURE_VCB)
#define GLES2_SCISSOR_VERTEX_SIZE_IN_BYTES	(GLES2_SCISSOR_VERTEX_FIRSTPHAS_SIZE_IN_BYTES + (EURASIA_USE_INSTRUCTION_SIZE * 2))
#else
#define GLES2_SCISSOR_VERTEX_SIZE_IN_BYTES	(GLES2_SCISSOR_VERTEX_FIRSTPHAS_SIZE_IN_BYTES)
#endif /* defined(SGX_FEATURE_VCB) */
#else
#define GLES2_SCISSOR_VERTEX_SIZE_IN_BYTES	(EURASIA_USE_INSTRUCTION_SIZE * 4)
...
************************************************************************************/
IMG_INTERNAL IMG_BOOL InitScissorUSECodeBlocks(GLES2Context *gc)
{	
	...
	gc->sPrim.psScissorVertexCodeBlock = UCH_CodeHeapAllocate(gc->psSharedState->psUSEVertexCodeHeap,
															GLES2_SCISSOR_VERTEX_SIZE_IN_BYTES);
	...
	GLES2_INC_COUNT(GLES2_TIMER_USECODEHEAP_VERT_COUNT, GLES2_SCISSOR_VERTEX_SIZE_IN_BYTES >> 2);
...
************************************************************************************/
IMG_INTERNAL IMG_VOID FreeScissorUSECodeBlocks(GLES2Context *gc)
{
...
IMG_INTERNAL IMG_VOID CalcRegionClip(GLES2Context *gc, EGLRect *psRegion, IMG_UINT32 *pui32RegionClip)
{
...
static GLES2_MEMERROR SetupVerticesForDrawmask(GLES2Context *gc,
										   EGLRect *psRect,
										   ...
										   IMG_UINT32 *pui32NumIndices)
{
	...
	*pui16Indices++	= 2;
	...
	if(ui32NumIndices == 3)
	{
		...
		*pfVertices++	= afClearSize[2];
		*pfVertices++	= 0;
		
		*pfVertices++	= 0;
		*pfVertices		= afClearSize[3];
	}
	else
	{
		*pfVertices++	= afClearSize[0];
		*pfVertices++	= afClearSize[1];
	...
	GLES2_INC_COUNT(GLES2_TIMER_INDEX_DATA_COUNT, ui32VertexDWords);
...
static GLES2_MEMERROR SetupVerticesAndShaderForDrawmask(GLES2Context *gc, EGLRect *psRect)
{
	...
	*pui32Buffer++ =	EURASIA_TAOBJTYPE_INDEXLIST	|
	...
	/* Index List 1 */
	*pui32Buffer++	= (uIndexAddr.uiAddr >> EURASIA_VDM_IDXBASE16_ALIGNSHIFT) << EURASIA_VDM_IDXBASE16_SHIFT;
	...
	*pui32Buffer++ =	(7 << EURASIA_VDMPDS_PARTSIZE_SHIFT) |
	...
	/* Index List 5 */
	*pui32Buffer   =	(1 << EURASIA_VDMPDS_VERTEXSIZE_SHIFT)				|
...
IMG_INTERNAL GLES2_MEMERROR SendDrawMaskRect(GLES2Context *gc, EGLRect *psRect, IMG_BOOL bIsEnable)
{
	/*
	...
	*/
	...
	/* Always send ispA, output selects (including texsize word + texfloat word), region clip
	...
	*/
	...
	{
		eError = PatchPregenMTEStateCopyPrograms(gc, (IMG_UINT32)(pui32Buffer - pui32BufferBase), uStateAddr);
	}
	else
	{
		eError = WriteMTEStateCopyPrograms(gc, (IMG_UINT32)(pui32Buffer - pui32BufferBase), uStateAddr);
	}	
	...
	*pui32Buffer++ = 0;
	...
	GLES2_INC_COUNT(GLES2_TIMER_MTE_STATE_COUNT, (pui32Buffer - pui32BufferBase));
...
IMG_INTERNAL GLES2_MEMERROR SendDrawMaskForPrimitive(GLES2Context *gc)
{
...
************************************************************************************/
IMG_INTERNAL GLES2_MEMERROR SendDrawMaskForClear(GLES2Context *gc)
{
	EGLDrawableParams *psDrawParams = gc->psDrawParams;
	EGLRect sScissor;
	GLES2_MEMERROR eError;
...
GL_APICALL void GL_APIENTRY glScissor(GLint x, GLint y, GLsizei width, GLsizei height)
{
	...
	GLES2_TIME_START(GLES2_TIMES_glScissor);

	if ((width < 0) || (height < 0))
	{
		SetError(gc, GL_INVALID_VALUE);
		GLES2_TIME_STOP(GLES2_TIMES_glScissor);
		return;
	}
