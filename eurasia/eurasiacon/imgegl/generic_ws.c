...
static IMG_HANDLE LoadNamedWSModule(KEGL_DISPLAY *pDisplay, char *pszWSModuleName, NativeDisplayType nativeDisplay)
{
	...
	{
		/*
		 * Get a function pointer through which we can get the function table.
		 */
		if (PVRSRVGetLibFuncAddr(hWSDrv,
								"WSEGL_GetFunctionTablePointer",
								(IMG_VOID **)&pfnWSEGL_GetFunctionTablePointer)!=PVRSRV_OK)
		{
			PVR_DPF ((PVR_DBG_ERROR,
				"LoadWSModule: Couldn't get address of jump table retrieval function for %s",
				pszWSModuleName));
...
IMG_INTERNAL IMG_BOOL LoadOGLES1AndGetFunctions(EGLGlobal *psGlobalData)
{
	...
	if (!hOGLModule)
	{
		PVR_DPF((PVR_DBG_ERROR, "LoadOGLES1AndGetFunctions: Couldn't load OGL module "OGLES1LIBNAME));
	...
	{
		PVR_DPF ((PVR_DBG_ERROR, "LoadOGLES1AndGetFunctions: Couldn't get address of glGetString"));
...
IMG_INTERNAL IMG_BOOL LoadOGLES2AndGetFunctions(EGLGlobal *psGlobalData)
{
	...
	if (!hOGLModule)
	{
		PVR_DPF((PVR_DBG_ERROR, "LoadOGLES2AndGetFunctions: Couldn't load OGL module "OGLES2LIBNAME));
	...
	if (PVRSRVGetLibFuncAddr(hOGLModule, "glGetString", (IMG_VOID **)&pfnglGetString)!=PVRSRV_OK)
	{
		PVR_DPF ((PVR_DBG_ERROR, "LoadOGLES2AndGetFunctions: Couldn't get address of glGetString"));
