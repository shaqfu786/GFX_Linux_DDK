...
#define EGL_FENCE_SYNC_COMPLETE(sync)									\
	((sync)->psSyncMemInfo->psClientSyncInfo->psSyncData->ui32WriteOpsPending \
	 == (sync)->psSyncMemInfo->psClientSyncInfo->psSyncData->ui32WriteOpsComplete)
...
static EGLBoolean _insertEglFence(KEGL_CONTEXT *ctx, TLS psTls, KEGL_SYNC *psSync)
{
	...
	eResult = SGXQueueTransfer(psSysContext->hTransferContext, &psSync->sQueueTransfer);
...
static EGLBoolean _deleteSync(KEGL_SYNC *psSync, SrvSysContext *psSysContext)
{
	...
	if(!EGL_FENCE_SYNC_COMPLETE(psSync))
	{
		return EGL_FALSE;
	}
...
static void _cleanupSyncs(KEGL_DISPLAY * psDpy, SrvSysContext *psSysContext)
{
	...
	while(psSync)
	{
		...
		{
			if(psSync->bIsDeleting && psSync->refcount==0 && EGL_FENCE_SYNC_COMPLETE(psSync))
			{
...
IMG_INTERNAL IMG_VOID TerminateSyncs(KEGL_DISPLAY * psDpy, SrvSysContext *psSysContext)
{
	...
	while(psSync)
	{
		...
		if(psSync->psDpy == psDpy)
		{
			...
			if(psSync->refcount>0 || !EGL_FENCE_SYNC_COMPLETE(psSync))
			{
...
static void _waitFence(KEGL_SYNC *psSync, SrvSysContext *psSysContext, EGLTimeKHR timeout)
{
	...
	hOSGlobalEvent = psSysContext->sHWInfo.sMiscInfo.hOSGlobalEvent;
#endif
#if defined(SUPPORT_VGX)
	hOSGlobalEvent = psSysContext->sVGXHWInfo.sMiscInfo.hOSGlobalEvent;
#endif
	...
	{
		/* Wait for an event to get posted. This call will swap out this thread. */
		/* TODO: Pending a services change - Currently this will wait a very long time for global events */

		if(PVRSRVEventObjectWait(psSysContext->psConnection, hOSGlobalEvent) != PVRSRV_OK)
		{
			PVR_DPF((PVR_DBG_WARNING, "IMGeglClientWaitSyncKHR: PVRSRVEventObjectWait timeout"));
		}
		...
		/* Check to see if the 3D has completed */
		if(EGL_FENCE_SYNC_COMPLETE(psSync))
		{
