...
IMG_INTERNAL IMG_BOOL SRV_SGXServicesInit(SrvSysContext *psSysContext, IMGEGLAppHints *psAppHints)
{
	...
	*/
	...
	psSysContext->sHWInfo.sMiscInfo.ui32StateRequest = PVRSRV_MISC_INFO_TIMER_PRESENT
														| PVRSRV_MISC_INFO_GLOBALEVENTOBJECT_PRESENT;

	if(PVRSRVGetMiscInfo(psSysContext->psConnection, &psSysContext->sHWInfo.sMiscInfo) != PVRSRV_OK)
	{
		PVR_DPF((PVR_DBG_ERROR,"Couldn't get Misc info"));
		goto ServInit_fail;
	}
	...
	*/
	...
	psSysContext->sRenderContext.ui32Flags = SGX_CREATERCTXTFLAGS_SHAREDPB;
	...
	psSysContext->sRenderContext.ui32PBSize = psAppHints->ui32ParamBufferSize;
	psSysContext->sRenderContext.ui32PBSizeLimit = psAppHints->ui32MaxParamBufferSize;
	...
	if(psSysContext->sHWInfo.sMiscInfo.ui32StatePresent
			& PVRSRV_MISC_INFO_GLOBALEVENTOBJECT_PRESENT)
	{
		psSysContext->sRenderContext.hOSEvent = psSysContext->sHWInfo.sMiscInfo.hOSGlobalEvent;
	}
	else
	{
		psSysContext->sRenderContext.hOSEvent = IMG_NULL;
	}
	...
	if(psSysContext->sHWInfo.sMiscInfo.ui32StatePresent
			& PVRSRV_MISC_INFO_GLOBALEVENTOBJECT_PRESENT)
	{
		sCreateTransfer.hOSEvent = psSysContext->sHWInfo.sMiscInfo.hOSGlobalEvent;
	}
	else
	{
...
{
	if (SGXDestroyTransferContext(psSysContext->hTransferContext, CLEANUP_WITH_POLL) != PVRSRV_OK)
	{
												  ...
												  EGLRenderSurface *psSurface)
{
	...
	IMG_UINT32 ui32MultiSample, ui32Align, ui32Size, ui32RTIndex;
	...
	/* Alloc dummy memory to get syncinfo. */
	if(IMGEGLALLOCDEVICEMEM(&psSysContext->s3D, psSysContext->hGeneralHeap, PVRSRV_MEM_READ,
								4, 4, &psSurface->psRenderSurfaceSyncMemInfo) != PVRSRV_OK)
	{
		goto fail_syncinfo_alloc;
	}

	/* Set up the sync info pointers.  psSyncInfo defaults to the sync info for the render surface */
	psSurface->psRenderSurfaceSyncInfo	= psSurface->psRenderSurfaceSyncMemInfo->psClientSyncInfo;
	psSurface->psSyncInfo				= psSurface->psRenderSurfaceSyncMemInfo->psClientSyncInfo;
	...
	psBuffer->hOSEvent					= psSysContext->sHWInfo.sMiscInfo.hOSGlobalEvent;
...
fail_statusval_alloc:
	IMGEGLFREEDEVICEMEM(&psSysContext->s3D, psSurface->psRenderSurfaceSyncMemInfo);
...
//SUPPORT_FORCE_FLIP_WORKAROUND
		...
		PVR_DPF((PVR_DBG_ERROR, "KEGL_SGXDestroyRenderSurface: Timeout failed on waiting for previous render op"));

		sMiscInfo.ui32StateRequest = PVRSRV_MISC_INFO_FORCE_SWAP_TO_SYSTEM_PRESENT;
		PVRSRVGetMiscInfo(psSysContext->psConnection, &sMiscInfo);
		 ...
		 */
		//PVRSRVClientEvent(PVRSRV_CLIENT_EVENT_HWTIMEOUT, &psSysContext->s3D, IMG_NULL);
...
//SUPPORT_FORCE_FLIP_WORKAROUND
	...
	if(IMGEGLFREEDEVICEMEM(&psSysContext->s3D, psSurface->psRenderSurfaceSyncMemInfo) != PVRSRV_OK)
	{
	...
	{
		if(IMGEGLFREEDEVICEMEM(&psSysContext->s3D, psSurface->psZSBufferMemInfo) != PVRSRV_OK)
		{
			PVR_DPF((PVR_DBG_ERROR,"KEGLDestroyRenderSurface: Couldn't free ZLS buffer"));
												  ...
												  EGLRenderSurface	*psSurface)
{
	...
	if(psSurface->psZSBufferMemInfo)
	{
		if(IMGEGLFREEDEVICEMEM(&psSysContext->s3D, psSurface->psZSBufferMemInfo) != PVRSRV_OK)
		{
			PVR_DPF((PVR_DBG_ERROR,"KEGL_SGXResizeRenderSurface: Couldn't free ZLS buffer"));
	...
	if(bHadZSBuffer || bCreateZSBuffer)
	{
		...
		if (IMGEGLALLOCDEVICEMEM(&psSysContext->s3D,
								...
								ui32SizeInBytes,
								EURASIA_PDS_DOUTT2_TEXADDR_ALIGNSHIFT,
								&psSurface->psZSBufferMemInfo) != PVRSRV_OK)
		{
													...
													IMG_UINT32			*pui32Stride)
{
	...
	*pui32Stride *= pixel_width;
	...
	return IMGEGLALLOCDEVICEMEM(&psSysContext->s3D,
								psSysContext->hGeneralHeap,
								PVRSRV_MEM_READ | PVRSRV_MEM_WRITE | PVRSRV_MEM_NO_SYNCOBJ,
								ui32Size,
								EURASIA_CACHE_LINE_SIZE,
								&psSurface->u.pbuffer.psMemInfo);
}
