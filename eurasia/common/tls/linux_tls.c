...
{
	return (IMG_UINT32)pthread_self();
}
...
IMG_VOID* TLS_PREFIX(_GetTLSValue(IMG_VOID))
{
    ...
    /* For details, please see notes at the top of this file. */
#if OPTIMISE_NON_NPTL_SINGLE_THREAD_TLS_LOOKUP
    if(bSingleThreaded)
    {
        return pvTLSThread0;
    ...
    /* Returns NULL if the key hasn't been created yet */
    return pthread_getspecific(TLSKey);

#else
    return pvTLS; /* USE_GCC__thread_KEYWORD */
#endif
}
...
IMG_BOOL TLS_PREFIX(_SetTLSValue(IMG_VOID *pvA))
{
    ...
    pthread_setspecific(TLSKey, pvA);

#if OPTIMISE_NON_NPTL_SINGLE_THREAD_TLS_LOOKUP
    if(bSingleThreaded)
    {
        if(pthread_self() == KeyCreateThreadID)
        {
            pvTLSThread0 = pvA;
        }
        else
        {
            /* Once it's been determined that we are multi-threaded, that's it,
             * we don't try and figure out if we later become single threaded.
             */
            bSingleThreaded = IMG_FALSE;
        }
    }
#endif

#else
    pvTLS = pvA; /* USE_GCC__thread_KEYWORD */
#endif
