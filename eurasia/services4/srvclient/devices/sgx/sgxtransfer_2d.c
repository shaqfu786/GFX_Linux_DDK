										   ...
										   SGX_QUEUETRANSFER *psQueueTransfer)
{
	...
	SGX_QUEUETRANSFER sQueueTransfer = *psQueueTransfer; // local copy because eType changes
	...
	switch (psQueueTransfer->eType)
	{
		...
		case SGXTQ_FILL:
		{
			...
			/* switch to 2D command type */
			sQueueTransfer.Details.s2DBlit.ui32CtrlSizeInDwords = ui32Bytes;
			sQueueTransfer.Details.s2DBlit.pui32CtrlStream = aui32ControlStream;
		...
		case SGXTQ_2DHWBLIT:
		{
			eError = Prepare2DCoreBlit(psTQContext, &sQueueTransfer,
					&(psSubmits->as2DSubmit[psSubmits->ui32Num2DSubmits]));
			if (eError != PVRSRV_OK)
			{
				goto Exit;
			}
