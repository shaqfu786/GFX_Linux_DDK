...
GLSL_EXPORT GLSLCompiledUniflexProgram * IMG_CALLCONV GLSLCompileToUniflex(GLSLCompileUniflexProgramContext *psCompileUniflexProgramContext)
{
	...
	pszCurrentLocale = GLSLSetLocale(IMG_NULL);

	/* Note that the storage pointed to by a pointer returend by
	 * setlocale() will be overwritten by successive calls to
	 * setlocale(), so we need to allocate our own store.
	 */
	if(pszCurrentLocale)
	{
		pszTemp = DebugMemAlloc(strlen(pszCurrentLocale)+1);

		if(!pszTemp)
		{
			LOG_INTERNAL_ERROR(("GLSLCompileUniflexProgram: Failed to alloc mem for current locale \n"));
			DebugMemFree(psGLSLCompiledUniflexProgram);
			return IMG_NULL;
		}
		strcpy(pszTemp, pszCurrentLocale);
		pszCurrentLocale = pszTemp;
	}
...
UniflexCleanUp:
	...
	if(pszCurrentLocale)
	{
		GLSLSetLocale(pszCurrentLocale);
		DebugMemFree(pszCurrentLocale);
	}
